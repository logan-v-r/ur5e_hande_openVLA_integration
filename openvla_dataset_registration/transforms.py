...
def ur5e_openvla_dataset_transform(
    trajectory: Dict[str, Any],
) -> Dict[str, Any]:
    """Standardize UR5e trajectories for OpenVLA.

    Raw RLDS state:
        [x, y, z, rx, ry, rz, gripper_closed]

    Raw RLDS action:
        [dx, dy, dz, drx, dry, drz, gripper_closed_target]

    Raw gripper convention:
        0.0 = open
        1.0 = closed

    Standardized OpenVLA action:
    ...
    tf.debugging.assert_equal(
        ...
        7,
        message=(
            "UR5e observation state must have 7 dimensions: "
            "[x, y, z, rx, ry, rz, gripper_closed]."
        ),
    )

    tf.debugging.assert_equal(
        tf.shape(raw_action)[-1],
        7,
        message=(
            "UR5e action must have 7 dimensions: "
            "[dx, dy, dz, drx, dry, drz, "
            "gripper_closed_target]. "
            "Check that TFDS version 2.1.0 is being loaded."
        ),
    )

    raw_gripper_closed = raw_action[:, 6:7]

    tf.debugging.assert_greater_equal(
        raw_gripper_closed,
        0.0,
        message="Raw gripper action must be at least 0.",
    )

    tf.debugging.assert_less_equal(
        raw_gripper_closed,
        1.0,
        message="Raw gripper action must be at most 1.",
    )

    # Convert the raw closed-target convention to OpenVLA's
    # open-target convention:
    #
    # raw 0=open  -> OpenVLA 1=open
    # raw 1=close -> OpenVLA 0=close
    openvla_gripper_open = 1.0 - raw_gripper_closed

    openvla_action = tf.concat(
        [
            raw_action[:, :6],
            openvla_gripper_open,
        ],
        axis=-1,
    )

    trajectory["observation"]["EEF_state"] = state[:, :6]

    # Preserve the recorded physical state convention:
    # 0=open and 1=closed.
    trajectory["observation"]["gripper_state"] = state[:, 6:7]

    trajectory["action"] = openvla_action
...
OXE_STANDARDIZATION_TRANSFORMS = {
    "ur5e_openvla": ur5e_openvla_dataset_transform,
